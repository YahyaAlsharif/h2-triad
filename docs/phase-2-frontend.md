...
The final Compose run's screenshots are in `frontend/test-results/compose/`. Examples: light desktop (local test artifact), mobile chart (local test artifact), and dark mobile dataset (local test artifact). These local artifacts are not committed and are regenerated by the test suite.
